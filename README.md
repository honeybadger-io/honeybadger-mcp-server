...
| `HONEYBADGER_API_KEY`             | no       | —                          | Project API key for usage analytics and error reporting. Hosted HTTP transport only — ignored in stdio mode, so a key set for your own app is never used. Unset disables analytics entirely. |
